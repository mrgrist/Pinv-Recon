...
acq_type = 'radial'; %'epi' 'radial' 'spiral'
FOV = 240;
gmax = 45;
smax = 140;
bw = 250d3;
...
switch acq_type
    case 'epi'
        flyback = 1;
        [k,dcf,t,grad,out,ga_burst] = design_epi(FOV,ntx,1,gmax,smax,...
            1,0,0,flyback,[],'1h',[1.1,1],bw,gdt);
        Kx0=k(1,:,1)*ntx; Ky0=k(1,:,2)*ntx; time0=t;
    case 'radial'
        type = 2; 
        [k,dcf,t,grad,ind,fname] = design_radial(FOV,ntx,(1/bw)*1d6,[],...
            gmax,smax,'1h',2,type,[1.5,1],1,1,0);
        Kx0=k(1,:,1)*ntx; Ky0=k(1,:,2)*ntx; time0=t;
    case 'spiral'
        arms = 1;
        [k,dcf,t,ind,out,grad] = design_spiral(FOV,ntx,arms,gdt*1d6,...
            [],gmax,smax,'1h',0,0,0);
        Kx0=real(k(1,:))*ntx; Ky0=imag(k(1,:))*ntx; time0=t;
end
...
% spatial discretization (centered)
xiLarge=linspace(-2,2,2*ntx+1);
xiLarge=xiLarge(1:end-1);
dri=mean(diff(xiLarge));
[XMlarge,YMlarge]=ndgrid(xiLarge,xiLarge);
ictr=ntx+(-ntx/2:ntx/2-1);
xiCtr=xiLarge(ictr);
[XMctr,YMctr]=ndgrid(xiCtr,xiCtr);

% Generating phantom
imgPhantom=phantom('Modified Shepp-Logan',ntx);
imgPhantom(imgPhantom>0.75)=0.75;
imgPhantom=single(imgPhantom/max(imgPhantom(:)));
imgLarge=zeros(2*ntx,2*ntx);
imgLarge(ictr,ictr)=imgPhantom;
imgCtr=interpn(XMlarge,YMlarge,imgLarge,XMctr,YMctr);
%=========================================
disp('>> 1. Reconstructing standard phantom');
%==============================================
% + shift
Kx=Kx0; Ky=Ky0; time=time0;
ENCODE00=single(exp(1i*pi*(Kx(:)*XMctr(:).'+Ky(:)*YMctr(:).')));
...
IMG00=reshape(RECON00*(ENCODE00*imgCtr(:)),[ntx,ntx]);
figure
imshow(mat2gray(abs(IMG00)),'InitialMagnification','fit')

%==============================================
disp('>> 2. Reconstructing with off centre');
%==============================================
...
ENCODE01=single(exp(1i*pi*(Kx(:)*XMoff(:).'+Ky(:)*YMoff(:).')));
tic, [U01,S01,V01]=svd((ENCODE01),'econ'); toc, U=U01; S=S01; V=V01; diagS01=diag(S01);
imax=find(diag(S)>max(diag(S))/CondNumb,1,'last'); invS=1./diag(S); invS(imax+1:end)=0; invS=diag(invS);
tic, RECON00=V*invS*U'; toc,
IMG01=reshape(RECON00*(ENCODE01*imgOff(:)),[ntx,ntx]);
figure
imshow(mat2gray(abs(IMG01)),'InitialMagnification','fit')

%==============================================
disp('>> 2. Reconstructing with non linear x gradient');
%==============================================
...
figure
imshow(mat2gray(abs(IMG10)),'InitialMagnification','fit')

figure
imshow(mat2gray(abs(IMG11)),'InitialMagnification','fit')

%==============================================
disp('>> 3. Reconstructing with off resonance');
%==============================================
...
figure
imshow(mat2gray(abs(IMG20)),'InitialMagnification','fit')

figure
imshow(mat2gray(abs(IMG21)),'InitialMagnification','fit')
...
switch acq_type
    case 'epi'
        ...
        [k,dcf,t,grad,out,ga_burst] = design_epi(FOV,ntx,1,gmax,smax,...
            1,0,0,flyback,[],'1h',[1.1,1],bw,gdt);
        Kx0=k(1,:,1)*ntx; Ky0=k(1,:,2)*ntx; time0=t;
        Kx=k(1,:,1)*ntx; Ky=k(1,:,2)*ntx; time=t*2;
        Kx1=k(1,:,1)*ntx; Ky1=k(1,:,2)*ntx; time0=t;
    case 'radial'
        [k,dcf,t,grad,ind,fname] = design_radial(FOV,ntx,(1/bw)*1d6,[],...
            gmax,smax,'1h',2,type,[1.5,1],1,3,0);
        Kx0=k(1,:,1)*ntx; Ky0=k(1,:,2)*ntx; time0=t;
        Kx=k(1,:,1)*ntx; Ky=k(1,:,2)*ntx; time=t*2;
        Kx1=k(1,:,1)*ntx; Ky1=k(1,:,2)*ntx; time0=t;
    case 'spiral'
        ...
        [k,dcf,t,ind,out,grad]=design_spiral(FOV,ntx,arms,gdt*1d6,...
            [],gmax,smax,'1h',0,0,0);
        Kx0=real(k(1,:))*ntx; Ky0=imag(k(1,:))*ntx; time0=t;
        Kx=real(k(1,:))*ntx; Ky=imag(k(1,:))*ntx; time=t*2;
        Kx1=real(k(1,:))*ntx; Ky1=imag(k(1,:))*ntx; time0=t;
end
...
%==============================================
disp('>> 3. Reconstructing with multi-channel');
%==============================================
...
figure
imshow(mat2gray(abs(IMG30)),'InitialMagnification','fit')

figure
imshow(mat2gray(abs(IMG31)),'InitialMagnification','fit')

%==============================================
disp('>> 3. Reconstructing with chemical-shift');
%==============================================
% + ChemShift
freqCS1=0; freqCS2=440; TE=1e-3;
...
figure
imshow(mat2gray(abs(IMG40)),'InitialMagnification','fit')

figure
imshow(mat2gray(abs(IMG42)),'InitialMagnification','fit')

%==============================================
disp('>> Plotting all results...');
%==============================================
figure; set(gcf,'Position',[104,4,1293,1076]); FntSz=12;
...
set(gca,'XTick',-ntx/2:ntx/4:ntx/2,'YTick',-ntx/2:ntx/4:ntx/2,'FontSize',FntSz), axis image, grid on, axis(20*[-1 1 -1 1]),
